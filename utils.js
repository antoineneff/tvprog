...
const ORDERED_CHANNELS = ['TF1', 'France 2', 'France 3', 'Canal+', 'France 5', 'M6', 'Arte', 'C8', 'W9', 'TMC', 'TFX', 'NRJ 12', 'France 4', 'CSTAR', "L'Equipe", '6ter', 'RMC Story', 'RMC Découverte', 'Chérie 25']
const MAX_CHANNEL_LENGTH = Math.max(...ORDERED_CHANNELS.map(channel => channel.length)) + 2

const channelsTemplate = ['tv/channel', {
  id: '@id',
  name: 'display-name'
}]
const programsTemplate = ['tv/programme', {
  title: 'title',
  channel: '@channel',
  start: '@start',
  end: '@stop',
...
async function filterPrograms({ channels, programs }) {
  const filteredPrograms = programs.filter(program => {
    const start = DateTime.fromFormat(program.start, 'yyyyMMddHHmmss ZZZ', { zone: 'Europe/Paris' })
    const end = DateTime.fromFormat(program.end, 'yyyyMMddHHmmss ZZZ', { zone: 'Europe/Paris' })

    return start.plus({ minutes: MINIMUM_PROGRAM_LENGTH }) < end && ((start.hour === 20 && start.minute > 45) || (start.hour === 21 && start.minute < 20))
  }).map(program => {
    return {
      ...program,
      start: DateTime.fromFormat(program.start, 'yyyyMMddHHmmss ZZZ').toMillis(),
...
export function print(programs) {
  ...
  if ((LINE_LENGTH - PROGRAM_TITLE.length) % 2 !== 0) {
    SPACES_AFTER_TITLE = ' '.repeat((LINE_LENGTH - PROGRAM_TITLE.length - 2) / 2 - 1)
  }
